        ...
        for k in range(self.maxIter):
            ...
            for n in range(self.ndims):
                ...
                for i in range(self.shape[n]):
                    ...
                    #因为服务器求逆一直出问题，修改了一下20191015
                    aaa = np.dot(np.linalg.inv(temp_uni),Mn[i, ].T)
                    #aaa = np.dot(np.linalg.pinv(temp_uni),Mn[i, ].T)
                    self.U[n][i,] = aaa.T
            ...
            # #2019/7/17 这里仅仅是为了看每一轮补全值和真实值的差距所以添加进来的，不然不需要每一轮都去补全X
            # if self.OnlyObs == True:
            #     midT = self.II.copy()
            #     for i in range(self.ndims):
            #         midT = midT.ttm(self.U[i], i + 1)
            ...
            if error < self.tol:
                ...
                # TH = np.percentile(self.X.data,99.99)
                # THidx = self.X.data>TH
                # self.X.data[THidx] =TH
