...
class Neg_binAirCP(object):
    ...
    def initializeLatentMatrices(self):
        ...
        np.random.seed(50)
        self.U = [np.random.randn(self.shape[i], self.rank) for i in range(self.ndims)]
    ...
    def run(self):
        ...
        for k in range(self.maxIter):
            ...
            def fir_dev(varx):
                tildep = 1/(1+np.exp(-varx))
                f1 = (self.failuresR+self.X.data)*tildep-self.failuresR
                return (f1)

            def sed_dev(varx):
                tildep = 1/(1+np.exp(-varx))
                f2 = self.failuresR*(1-tildep)
                return (f2)
            ...
            if error < self.tol :
                ...
                if self.OnlyObs == True:
                    ...
                    self.X = midT.copy()
                    self.X.data = self.T.data * self.omega.data + (self.failuresR*np.exp(-self.X.data)) * (1 - self.omega.data)

                TH = np.percentile(self.X.data,99.9)
