...
class BernoulliAirCP(object):
    """ This routine solves the Exponential_Family_Air_CP Tensor
      completion via Alternation Direction Method of Multipliers (ADMM)."""

    def __init__(self, obser,binomialn, omega=None, rank=4, tol=1e-5, max_iter=500, sim_mats=None, alpha=None, lmbda=None,
    ...
    def run(self):
        ...
        for k in range(self.maxIter):
            ...
            def fir_dev(varx):
                f1 = self.binomialn*np.exp(varx)/(1+np.exp(varx))
                return (f1)

            def sed_dev(varx):
                temmp = np.exp(varx)
                f2 = self.binomialn*temmp/np.power(1+temmp,2)
                return (f2)
